...
exports.list = async (query) => {
  return new Movie().query((qb) => {
    ...
    // filter by location
    if (query.location) {
      qb.innerJoin('locations_movies', 'movies.id', 'locations_movies.movie_id');
      qb.innerJoin('locations', 'locations.id', 'locations_movies.location_id');
      qb.where('locations.name', query.location);
    }
