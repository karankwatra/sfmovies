...
exports.addLocationToMovie = async (movie_id, location_name) => {
  const movie = await new Movie({ id: movie_id }).fetch({ withRelated: ['locations'] });
  const location = await new Location({ name: location_name }).fetch();
  ...
  if (!location) {
    const new_location = await new Location({ name: location_name }).save();
    movie.locations().attach(new_location);
  } else {
  ...
  return new Movie({ id: movie_id }).fetch({ withRelated: ['locations'] });
};
